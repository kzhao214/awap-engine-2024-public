...
class BotPlayer(Player):
    def __init__(self, map: Map):
        self.map = map
        self.sniperpositions = self.mostpath(60, True, self.map.width*self.map.height)
        self.bomberpositions = self.mostpath(10, True, self.map.width*self.map.height)
        self.sunpositions = self.mostpath(60, False, self.map.width*self.map.height)
    ...
    def build_towers(self, rc: RobotController):
        ...
        if(max(self.bomberpositions)*3>max(self.sniperpositions)):
